...
class PokerHand
  ...
  def evaluate
    case
    when self.class.royal_flush?(cards)     then 'Royal flush'
    when self.class.straight_flush?(cards)  then 'Straight flush'
    when self.class.four_of_a_kind?(cards)  then 'Four of a kind'
    when self.class.full_house?(cards)      then 'Full house'
    when self.class.flush?(cards)           then 'Flush'
    when self.class.straight?(cards)        then 'Straight'
    when self.class.three_of_a_kind?(cards) then 'Three of a kind'
    when self.class.two_pair?(cards)        then 'Two pair'
    when self.class.pair?(cards)            then 'Pair'
    else                       'High card'
    end
  end
