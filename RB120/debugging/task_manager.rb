class TaskManager
  ...
  def complete_task(task_name)
    ...
    if completed_task
      ...
      #potential error here, should be task_name in #{} - no, this is fine because
      #completed_task is assigned to a Task object and therefore has a name
    else
      puts "Task not found."
    end
  end
  ...
  def display_high_priority_tasks
    urgent_tasks = tasks.select do |task| #potential error here - creating new variable tasks
      task.priority == :high
    end

    display(urgent_tasks) # then displaying original tasks 
  end
